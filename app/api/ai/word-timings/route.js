...
function getPublicErrorMessage(error) {
  ...
  return "Word timing transcription failed unexpectedly.";
}
...
export async function POST(request) {
  ...
  if (!audioAssetId) {
    return NextResponse.json(
      { error: "Upload an MP3 before loading word timings." },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    if (metadata.kind !== "audio") {
      return NextResponse.json(
        { error: "Choose an uploaded MP3 before loading word timings." },
        { status: 400 },
      );
    }
