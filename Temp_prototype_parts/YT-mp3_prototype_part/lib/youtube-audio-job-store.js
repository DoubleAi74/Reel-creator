...
const REUSABLE_JOB_MS = 10 * 60 * 1000;

const globalStore = globalThis.__youtubeAudioSegmentJobStore || {
  jobs: new Map(),
  fingerprints: new Map(),
};

globalThis.__youtubeAudioSegmentJobStore = globalStore;
...
export function markJobFailed(jobId, errorCode) {
  const job = getJob(jobId);
...
export function publicJob(job) {
  ...
  if (job.status === "complete") {
    response.downloadUrl = `/api/youtube-audio-segments/${job.id}/file`;
  }
