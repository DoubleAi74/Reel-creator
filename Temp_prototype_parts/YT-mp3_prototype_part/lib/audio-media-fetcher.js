...
} from "./providers/rapidapi-client.js";
...
async function fetchMediaUrl(media, options) {
  ...
  if (!response.ok) {
    ...
    logDiagnostic(
      ...
      {
        target: summarizeUrl(url),
        finalTarget: summarizeUrl(response.url || url.toString()),
        depth: options.depth,
  ...
  if (isJsonLikeContentType(contentType)) {
    if (options.depth >= MAX_REDIRECT_DEPTH) {
  ...
  if (looksLikeImageUrl(response.url || url.toString()) || contentType.startsWith("image/")) {
    logDiagnostic(
      "media-image-response",
      {
        target: summarizeUrl(response.url || url.toString()),
        contentType,
      },
      "warn",
    );
  ...
  return {
    ...
    sourceHost: new URL(response.url || url.toString()).hostname,
    quotaHeaders: responseQuotaHeaders,
  };
}
...
async function rejectLocalNetworkHost(hostname) {
  ...
  if (directIpVersion && isPrivateIp(hostname, directIpVersion)) {
