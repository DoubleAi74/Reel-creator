...
- **`components/waveform-timeline.js`** — the transport + waveform React component.
  ...
  - The **lyric-marker render** (context only — you likely will NOT change JSX here): **lines 709–737**. Each marker renders a `<span className="waveform-marker-cap" />` + `<span className="waveform-marker-line" />` inside a `.waveform-marker.waveform-marker--{state}` div, where `state` is `active | heard | idle` (computed lines 714–721). Horizontal position is `left: ${getMarkerLeftPercent(line.start, audio)}%` (around line 729) — **leave this untouched.**
- **`app/globals.css`** — the waveform + marker styling you will edit.
  - **Waveform internals** (`.transport .waveform*`), all-width/unlayered: the main block starts around **line 1297** (after the mobile-only rules). The two rules that control the waveform's on-screen height/centering are `.transport .waveform-engine-layer` (`position:absolute; inset:0; display:flex; align-items:center`, ~line 1327) and `.transport .waveform-canvas` (the WaveSurfer container, `height:100%`, ~line 1336) — these are what you constrain for Task 3.
  - **Lyric marker rules** (`.transport .waveform-marker*`), all-width/unlayered: start around **line 1399** (`.waveform-marker`), with the line and cap rules at **~1407** and **~1417** — this is the main block for Tasks 4–6.
  - **Skeleton** (loading placeholder bars, `.waveform-skeleton*`): around **lines 1352–1390** — optional note in §6.
...
- **Remove** the three bar options so the renderer switches to the filled line waveform:
  - delete `barGap: 2,` (near line 348)
  - delete `barRadius: 999,` (near line 349)
  - delete `barWidth: 2,` (near line 350)
...
File: `app/globals.css`, waveform internals (main unlayered block starting ~line 1297; target the canvas rule near line 1336).

The waveform currently fills the whole scrub-bar height: `.transport .waveform-engine-layer` is `position:absolute; inset:0; display:flex; align-items:center` (~line 1327) and `.transport .waveform-canvas` (the WaveSurfer container) is `height: 100%` (~line 1336). WaveSurfer's `height:"auto"` fills whatever height that container has.
...
File: `app/globals.css`, marker block (starts ~line 1399; key rules near 1407 for the line and 1417 for the cap).
...
File: `app/globals.css`, marker block (idle cap rule near line 1432).

- In `.transport .waveform-marker--idle .waveform-marker-cap` (near lines 1432, which sets `background`), **hide the dot**: set `display: none;` (you can drop the now-unused `background` line, or leave it — `display:none` wins).
- Leave the `--heard` cap and `--active` cap (the rules immediately following the idle ones) as they are — those dots stay.
- **Note:** the JSX still renders the cap `<span>` for every marker (around line 731); hiding idle via CSS is intentional and keeps the render uniform — do **not** change the JSX to conditionally omit it.

### Task 6 — De-emphasize idle contrast
File: `app/globals.css`, marker block (idle line rule near line 1428).

- In `.transport .waveform-marker--idle .waveform-marker-line` (near line 1428), lower the idle colour so idle marks are clearly quieter than active (green) / heard (mauve):
...
- **Risk:** idle tick too tall/dark still competes with the waveform. Mitigation: tune `height` (Task 3) and alpha (Task 6) by eye.
...
- base line ~1407: `.waveform-marker-line { top:7px; bottom:5px; left:50%; width:1.5px; transform:translateX(-50%); border-radius:999px; }`
- base cap ~1417: `.waveform-marker-cap { top:1px; left:50%; width:5px; height:5px; transform:translateX(-50%); border-radius:999px; box-shadow:0 0 0 1.5px var(--surface); }`
- idle line ~1428: `background: hsl(28 8% 34% / 0.45);` → **edit here (Task 6)**
- idle cap ~1432: `background: hsl(28 8% 34% / 0.7);` → **edit here (Task 5: display:none)**
- heard line/cap and active line/cap (following rules): **leave unchanged.**
