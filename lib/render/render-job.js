...
import { resolveAssetStorage } from "@/lib/files";
...
import {
  createSessionAssetSignedReadUrl,
  downloadR2ObjectToFile,
  extensionForVideoContentType,
} from "@/lib/r2/session-asset-lifecycle";
...
async function resolveSessionAsset({
  assetId,
  expectedKinds,
  jobId,
  label,
  ...
  try {
    ...
    const ext =
      metadata.kind === "video"
        ? extensionForVideoContentType(metadata.mimeType, metadata.name)
        : metadata.kind === "image"
          ? ".png"
          : ".bin";
    const workDir = path.join(os.tmpdir(), `reel-render-${jobId}`);
    await mkdir(workDir, { recursive: true });
    const filePath = path.join(workDir, `bg-${assetId}${ext}`);

    await downloadR2ObjectToFile({
      key: resolved.r2ObjectKey,
      outputPath: filePath,
    });
    ...
    try {
      const signed = await createSessionAssetSignedReadUrl({
        key: resolved.r2ObjectKey,
      });
