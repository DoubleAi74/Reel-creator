...
export const MICROS_PER_MINOR = 1_000_000;
// SumUp live online checkouts effectively require ≥ £1; keep app min in sync.
...
export function roundMicrosToPenceHalfUp(rawCostMicros) {
  if (!Number.isSafeInteger(rawCostMicros) || rawCostMicros < 0) {
    throw new Error("Invalid micro-pence amount.");
  }

  return Math.floor((rawCostMicros + MICROS_PER_MINOR / 2) / MICROS_PER_MINOR);
}
