...
export function getExportReadiness({
  audioAssetId,
  /** Playable blob/https URL required for browser (client) export */
  audioObjectUrl = null,
  backgroundAssetId,
  ...
  if (requirePlayableAudio && !audioObjectUrl) {
    return {
      canExport: false,
      reason:
        "Playable audio is missing. Re-upload the MP3 or re-convert from YouTube in this session.",
    };
  }
