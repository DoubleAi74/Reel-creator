...
export async function refreshPaymentOrderFromSumUp(order) {
  ...
  if (checkout.status !== "PAID") {
    const updatedOrder = await markOrderFromCheckout(order, checkout);
    const transactionSummaries = Array.isArray(checkout.transactions)
    ...
    console.info("[payments:verify] checkout-not-paid", {
      amountMinor: order.amountMinor,
      balanceCredited: false,
      checkoutStatus: checkout.status,
      mappedStatus: mapCheckoutStatusToOrderStatus(checkout.status),
      orderId: order.publicReference,
  ...
  if (!verification.ok) {
    console.info("[payments:verify] verification-mismatch", {
  ...
  console.info("[payments:verify] paid", {
    amountMinor: order.amountMinor,
    credited,
    orderId: order.publicReference,
    sumupCheckoutId: order.sumupCheckoutId,
    transactionId: getCheckoutTransactionId(checkout) ?? null,
  });
