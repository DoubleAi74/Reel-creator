...
function normalizeMode(value) {
  const normalizedMode = String(value ?? "sandbox").trim().toLowerCase();

  return normalizedMode === "test" ? "sandbox" : normalizedMode;
}
...
function isPublicOrAffiliateKey(value) {
  return value.startsWith("sup_pk_") || value.startsWith("sup_afk_");
}

function isSandboxSecretKey(value) {
  return (
...
function isGenericServerSecretKey(value) {
  return (
    value.startsWith("sup_sk_") &&
    !value.startsWith("sup_sk_live_") &&
    !value.startsWith("sup_pk_") &&
