...
export async function transcribeAndCleanLyrics({
  ...
  const transcriptionText = await requestContentTranscriptionResilient({
    audioSeconds: getPositiveDurationSeconds(audio?.duration),
    contentType,
    fileBuffer,
    fileName,
    onProgress,
...
async function requestTimestampedTranscriptionFromChunks({
  ...
  if (!chunks || chunks.length === 0) {
    return requestTimestampedTranscription({
      audioSeconds: getPositiveDurationSeconds(
        getDurationSeconds(timingAudio.windowStart, timingAudio.windowEnd),
        audio?.duration,
      ),
