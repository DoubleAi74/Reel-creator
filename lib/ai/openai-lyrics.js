...
import { recordOpenAiCallUsage } from "./openai-usage.js";
...
/**
 * Live models the lyric pipeline can call, grouped by billing phase (REP-206).
 * Env overrides are resolved at call time so precheck matches runtime.
 */
export function getLiveOpenAiModelsByBillingPhase() {
  return {
    enrich: [
      TRANSLATION_MODEL,
      process.env.OPENAI_LYRIC_POLISH_MODEL ?? DEFAULT_LYRIC_POLISH_MODEL,
    ...
    time: [
      TIMESTAMP_TRANSCRIPTION_MODEL,
      process.env.OPENAI_QA_AUDIT_MODEL ?? "gpt-4o-mini",
    ],
    transcribe: [
      CONTENT_TRANSCRIPTION_MODEL,
      LINE_BREAK_MODEL,
      process.env.OPENAI_SOURCE_REPAIR_MODEL ?? "gpt-5.4-mini",
    ],
  };
...
// Length of each audio segment sent to the *timestamped* (whisper) path.
// Smaller chunks transcribe fast and make transient 5xx retries cheap.
const TRANSCRIPTION_CHUNK_SECONDS = Math.max(
  10,
  Number(process.env.OPENAI_TRANSCRIPTION_CHUNK_SECONDS ?? 45),
);
// Content (gpt-4o-transcribe) uses non-overlapping windows so hard encodes
// (e.g. YouTube CDN masters) still yield full-song lyrics. Whole-file single
// calls often return only a short vocal fragment on those files. Default 2 min
// balances coverage vs cross-line context for the line formatter.
const CONTENT_CHUNK_SECONDS = Math.max(
  10,
  Number(process.env.OPENAI_CONTENT_CHUNK_SECONDS ?? 120),
);
...
// gpt-4o-transcribe accepts up to 25 MB per request. Content path prefers
// fixed-length chunks (see CONTENT_CHUNK_SECONDS); this cap only bounds a
// single-call fallback when the file is short or chunking fails.
const CONTENT_SINGLE_CALL_MAX_BYTES = Number(
  process.env.OPENAI_CONTENT_SINGLE_CALL_MAX_MB ?? 24,
) * 1024 * 1024;
// Per content request timeout (one chunk or one short whole-file call).
  ...
  for (let index = 0; index < windows.length; index += 1) {
    ...
    try {
      ...
      const result = await requestTimestampedTranscription({
        ...
        audioSeconds: getDurationSeconds(window.start, window.end),
...
function createLyricLineId(id) {
  return typeof id === "string" && id.trim() ? id : randomUUID();
}

function ensureCanonicalLineIds(lines) {
  return lines.map((line, index) => ({
    ...line,
    id: createLyricLineId(line?.id),
    index: Number.isInteger(line?.index) ? line.index : index + 1,
  }));
}

function normalizeMeaningOutputWords(words) {
  return (Array.isArray(words) ? words : [])
    .map((word) => {
      const text = cleanTextField(word?.text ?? word?.word);
      ...
      return {
        ...
        roman: cleanTextField(word?.roman),
        text,
      };
    })
    .filter(Boolean);
...
export async function transcribeAndCleanLyrics({
  ...
  if (lockedLines.length > 0) {
    const linesWithIds = ensureCanonicalLineIds(lockedLines);

    notifyProgress(onProgress, {
      detail: `Using ${linesWithIds.length} existing lyric line${
        linesWithIds.length === 1 ? "" : "s"
  ...
  const transcriptionText = await requestContentTranscriptionResilient({
    audioSeconds: await resolveBillingAudioSeconds({
      assetDurationSec,
      audioFilePath,
      projectDurationSec: audio?.duration,
      requestDurationSec: audio?.duration,
    }),
    contentType,
    fileBuffer,
    fileName,
  ...
  return {
    lines: repairedLyricLineInputs.map((line, index) => ({
      id: createLyricLineId(line.id),
      index: Number.isInteger(line?.index) ? line.index : index + 1,
...
async function buildCanonicalLyricSet({
  ...
  const cleanedLyricSet = await transcribeAndCleanLyrics({
    assetDurationSec,
    audio,
    audioFilePath,
    contentType,
  ...
  if (cleanedLyricSet.source === "user") {
    return cleanedLyricSet;
  }

  const lyricLineInputs = canonicalLinesToLyricInputs(cleanedLyricSet.lines);

  notifyProgress(onProgress, {
    detail: `Translating ${lyricLineInputs.length} lyric line${
      lyricLineInputs.length === 1 ? "" : "s"
    } to English.`,
    stage: "translating",
  ...
  return {
    lines: translatedLines.map((line, index) => ({
      id: createLyricLineId(cleanedLyricSet.lines[index]?.id),
      index: index + 1,
      original: line.original,
    ...
    source: cleanedLyricSet.source,
    sourceRepairSummary: cleanedLyricSet.sourceRepairSummary,
  };
}
...
export async function enrichLyricLines({
  ...
  const lyricLineInputs = normalizeTranscriptLineInput(lines);

  if (lyricLineInputs.length === 0) {
    throw new Error("Add lyric lines before translating and enriching.");
  }
  ...
  const canonicalLines = translatedLines.map((line, index) => ({
    id: createLyricLineId(lyricLineInputs[index]?.id),
    index: lyricLineInputs[index]?.index ?? index + 1,
    original: lyricLineInputs[index]?.original ?? line.original,
    romanization: line.romanization ?? "",
    start: null,
    translation: line.translation ?? "",
  }));
  ...
  notifyProgress(onProgress, {
    detail: `Enriched ${linesWithMeanings.length} lyric line${
      linesWithMeanings.length === 1 ? "" : "s"
    }.`,
    stage: "complete",
  ...
  return {
    lineCount: linesWithMeanings.length,
    lines: linesWithMeanings.map((line) => ({
      id: createLyricLineId(line?.id),
      romanization: cleanTextField(line?.romanization),
      translation: cleanTextField(line?.translation),
      words: normalizeMeaningOutputWords(line?.words),
    })),
...
export async function generateLyricLines({
  assetDurationSec = null,
  audio,
  audioFilePath = null,
  contentType,
  ...
  const enrichedLyricSet = await enrichLyricLines({
    includeRomanization,
    lines: cleanedLyricSet.lines,
    onProgress,
    sourceLanguage,
    usageCollector,
  });
  const enrichedLinesById = new Map(
    (Array.isArray(enrichedLyricSet.lines) ? enrichedLyricSet.lines : [])
      .filter((line) => typeof line?.id === "string" && line.id)
      .map((line) => [line.id, line]),
  );
  const lines = cleanedLyricSet.lines.map((line) => {
    const enrichedLine = enrichedLinesById.get(line.id);

    return {
      id: createLyricLineId(line?.id),
      original: cleanTextField(line?.original),
      romanization: cleanTextField(enrichedLine?.romanization),
      start: null,
      translation: cleanTextField(enrichedLine?.translation),
      words: normalizeMeaningOutputWords(enrichedLine?.words),
    };
  });

  notifyProgress(onProgress, {
    detail: `Generated ${lines.length} lyric line${
      lines.length === 1 ? "" : "s"
    } with translations and word meanings.`,
    stage: "complete",
  ...
  return {
    ...
    lines,
    lyricPolishSummary: enrichedLyricSet.lyricPolishSummary,
    source: cleanedLyricSet.source,
    sourceLanguage: sourceLanguage.label,
    sourceRepairSummary: cleanedLyricSet.sourceRepairSummary,
    wordMeaningsSummary: enrichedLyricSet.wordMeaningsSummary,
  };
}

export async function timeLyricLinesFromAudio({
  assetDurationSec = null,
  audio,
  audioFilePath = null,
  contentType,
  ...
  const lyricLineInputs = ensureCanonicalLineIds(
    normalizeTranscriptLineInput(lines),
  );

  if (lyricLineInputs.length === 0) {
    throw new Error("Add lyric lines before timing.");
  }
  ...
  const outputLines = lyricLineInputs.map((line, index) => {
    ...
    return {
      ...
      original: line.original,
      ...timingFields,
      start: hasUsableLineTiming(match) ? match.start : null,
    };
  });
  ...
  notifyProgress(onProgress, {
    detail: `Matched timing for ${timingSummary.matchedCount} lyric line${
      timingSummary.matchedCount === 1 ? "" : "s"
    }.`,
    stage: "complete",
    title: "Timing complete",
  });
  ...
  return {
    ...
    lineCount: outputLinesWithQuality.length,
    lines: outputLinesWithQuality.map((line) => ({
...
async function requestContentTranscription({
  ...
  await recordOpenAiCallUsage({
    ...
    data,
    endpointKind: "audio",
    fallbackAudioSeconds: audioSeconds,
    model: CONTENT_TRANSCRIPTION_MODEL,
    phase: "transcribe",
...
// Accurate lyric content via gpt-4o-transcribe.
// Prefer fixed ~2 min non-overlapping chunks for any track longer than one chunk
// (overlap would duplicate lyrics; there are no word timings to stitch here).
// Short files (or failed splits) fall back to a single call within the API size cap.
async function requestContentTranscriptionResilient({
  ...
  try {
    chunks = await splitAudioIntoChunks({
      chunkSeconds: CONTENT_CHUNK_SECONDS,
      fileBuffer,
      fileName,
      overlapSeconds: 0,
    });
  ...
  // splitAudioIntoChunks returns [] when duration ≤ chunkSeconds (or probe fails).
  if (!chunks || chunks.length === 0) {
    if (fileBuffer.length > CONTENT_SINGLE_CALL_MAX_BYTES) {
      throw new Error(
        "Audio is too large for a single transcription request and could not be split into chunks.",
      );
    }
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const text = await requestContentTranscription({
      audioSeconds: getDurationSeconds(chunk.start, chunk.end),
...
function getPositiveDurationSeconds(...candidates) {
...
/**
 * REP-204 billing duration fallback chain:
 * request/segment duration → asset metadata.durationSec → project audio duration → ffprobe.
 */
export async function resolveBillingAudioSeconds({
  assetDurationSec = null,
  audioFilePath = null,
  ...
  const fromCandidates = getPositiveDurationSeconds(
    requestDurationSec,
    segmentDurationSec,
    assetDurationSec,
    projectDurationSec,
  ...
  if (audioFilePath) {
    try {
      const { probeMediaDurationSec } = await import("../files.js");
      return await probeMediaDurationSec(audioFilePath);
    } catch {
      return null;
    }
  }
  ...
  if (!chunks || chunks.length === 0) {
    return requestTimestampedTranscription({
      audioSeconds: await resolveBillingAudioSeconds({
        ...
        projectDurationSec: audio?.duration,
        requestDurationSec: audio?.duration,
        segmentDurationSec: getDurationSeconds(
          timingAudio.windowStart,
...
async function requestLyricLineBreaks({ sourceLanguage, text, usageCollector }) {
  const response = await fetchOpenAiWithRetry(
    RESPONSES_URL,
    {
      body: JSON.stringify({
...
async function requestLyricSourceRepair({
  lines,
  sourceLanguage,
  transcriptionText,
  usageCollector,
...
// pipeline coverage fill and staged enrichment.
