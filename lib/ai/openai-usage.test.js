...
describe("OpenAI usage collector", () => {
  ...
  it("does not write a phantom-cost UsageRecord when responseOk is false (REP-203)", async () => {
    const collector = createUsageCollector({
      jobId: "job-usage-fail",
      pipelineRunId: "run-usage-fail",
    });

    const failed = await recordOpenAiCallUsage({
      collector,
      data: {},
      endpointKind: "audio",
      fallbackAudioSeconds: 120,
      model: "whisper-1",
      phase: "time",
      responseOk: false,
    });
    ...
    expect(await UsageRecord.countDocuments()).toBe(0);
    expect(collector.serialize().calls).toEqual([]);

    const ok = await recordOpenAiCallUsage({
      collector,
      data: { duration: 12 },
      endpointKind: "audio",
      model: "whisper-1",
