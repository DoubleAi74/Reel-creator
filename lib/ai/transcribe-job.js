...
/**
 * Vercel (and multi-instance hosts) cannot poll in-memory jobs across isolates.
 * Run the job inline in the POST request so the same process returns the result.
 * Override with TRANSCRIBE_SYNC=0/1.
 */
export function shouldRunTranscribeJobsSynchronously() {
  if (
    process.env.TRANSCRIBE_SYNC === "1" ||
  ...
  if (
    ...
    process.env.TRANSCRIBE_SYNC === "false"
  ) {
    return false;
  }
