...
export function findInFlightTranscribeForSession(sessionId, assetId, phase = null) {
...
export function markTranscribeJobAccounting(
  jobId,
  {
    balanceExhausted = false,
    error = null,
  ...
  return updateTranscribeJob(jobId, {
    ...
    writeOffMinor:
      Number.isInteger(writeOffMinor) && writeOffMinor >= 0 ? writeOffMinor : 0,
  });
}
...
export function toTranscribeJobResponse(job) {
  ...
  return {
    assetId: job.assetId,
    accountingError: job.accountingError,
    accountingStatus: job.accountingStatus,
    balanceExhausted: job.balanceExhausted === true,
    detail: job.detail,
    error: job.error,
    phase: job.phase,
    ...
    title: job.title,
    writeOffMinor:
      Number.isInteger(job.writeOffMinor) && job.writeOffMinor >= 0
        ? job.writeOffMinor
        : 0,
