...
describe("autosave envelope", () => {
  ...
  it("round-trips the project, audio descriptor, and transcription pointer", () => {
    const raw = encodeAutosave({
      ...
      transcription: { appliedJobId: null, jobId: "job-1", phase: "generate" },
    });
    ...
    expect(decoded.generationSave).toMatchObject({
      assetId: "asset-1",
      finalJobId: "job-final",
      pipelineRunId: "run-1",
      status: "ready",
  ...
  it("still decodes v2 envelopes without generationSave", () => {
    const envelope = JSON.parse(encodeAutosave({ project: sampleProject() }));
    delete envelope.generationSave;

    const decoded = decodeAutosave(JSON.stringify(envelope));
    expect(decoded.generationSave.status).toBe("idle");
  });

  it("maps legacy transcription modes to staged phases", () => {
    const envelope = JSON.parse(encodeAutosave({ project: sampleProject() }));
    ...
    expect(decodeAutosave(JSON.stringify(envelope)).transcription).toEqual({
      appliedJobId: "job-2",
      jobId: "job-2",
      phase: "time",
    });
  ...
  it("drops a transcription pointer that has no jobId", () => {
    const raw = encodeAutosave({
      project: sampleProject(),
      transcription: { appliedJobId: "x", phase: "time" },
    });
