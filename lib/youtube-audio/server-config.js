...
export function getYoutubeAudioConfig() {
  ...
  // ytjar (youtube-mp36) requires RapidAPI username in the media User-Agent
  // or CDN links return HTTP 404. See YOUTUBE_AUDIO_SETUP.md.
  const rapidApiUsername =
    process.env.RAPIDAPI_USERNAME?.trim() ||
    process.env.YT_MP36_RAPIDAPI_USERNAME?.trim() ||
    "";
