...
import { buildMp36MediaHeaders } from "./providers/youtube-mp36";
...
describe("youtube audio Stage 0 scaffold", () => {
  ...
  it("appends RapidAPI username to youtube-mp36 media User-Agent", () => {
    const headers = buildMp36MediaHeaders({ rapidApiUsername: "demo_user" });
    expect(headers["User-Agent"]).toContain("demo_user");
    expect(headers.Accept).toMatch(/audio/);
  });
