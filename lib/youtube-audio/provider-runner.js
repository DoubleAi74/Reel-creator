import { AudioProcessingError, canUseLocalFfmpeg } from "./audio-ffmpeg";
...
const NON_FALLBACK_ERROR_CODES = new Set([
  ...
  "INVALID_PROVIDER",
  "SEGMENT_TOO_LONG",
  // Do not waste ~40s falling back into another dead end when binaries are gone.
  "FFMPEG_MISSING",
]);
...
export async function runProviderSegmentJob(input, job, options = {}) {
  ...
  if (providerPlan.length === 0) {
    throw new AudioProcessingError(
      "No YouTube audio providers available without local ffmpeg/ffprobe. On Vercel use auto/youtube-mp36 (provider-trimmed MP3) or set FFMPEG_PATH and FFPROBE_PATH.",
      { errorCode: "FFMPEG_MISSING" },
    );
  }

  logDiagnostic("provider-plan", {
    jobId: job.id,
    requestedProviderId: input.providerId,
    plan: providerPlan,
    localFfmpeg: canUseLocalFfmpeg(),
  });
...
export function providerPlanForInput(input) {
  ...
  if (
    ...
    plan = [...YOUTUBE_AUDIO_FALLBACK_PROVIDER_IDS];
  } else {
    plan = [input.providerId];
  }

  // Local-trim providers need ffmpeg. On Vercel (and similar) skip them so auto
  // does not burn time on youtube-mp3-2025 only to fail with FFMPEG_MISSING,
  // which hides the real youtube-mp36 error.
  if (!canUseLocalFfmpeg()) {
    plan = plan.filter((providerId) => !providerNeedsLocalFfmpeg(providerId));
  }

  return plan;
}

export function providerNeedsLocalFfmpeg(providerId) {
  const option = getYoutubeAudioProviderOption(providerId);
  return option?.trimMode === "local";
}
...
function userMessageForErrorCode(errorCode) {
  switch (errorCode) {
    ...
    case "FFMPEG_MISSING":
      return "Server is missing ffmpeg/ffprobe for local audio trim.";
    case "RESULT_EXPIRED":
