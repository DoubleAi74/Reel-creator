...
async function fetchMediaUrl(media, options) {
  ...
  if (!response.ok) {
    ...
    const statusHint =
      response.status === 404
        ? " Media CDN returned 404 — for youtube-mp36 set RAPIDAPI_USERNAME (RapidAPI account username) so the download User-Agent is whitelisted."
        : "";

    throw new AudioMediaFetchError(
      `Media URL returned HTTP ${response.status}${statusHint}`,
      {
        errorCode:
          response.status === 410
            ? "RESULT_EXPIRED"
