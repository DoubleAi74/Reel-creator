...
import { extractRapidApiQuotaHeaders } from "../rapidapi-quota";
...
export async function rapidApiFetch(url, init, options = {}) {
  ...
  try {
    response = await fetch(url, {
      ...
      cache: "no-store",
      signal:
        init?.signal || AbortSignal.timeout(options.timeoutMs || DEFAULT_TIMEOUT_MS),
    });
  } catch (error) {
    logDiagnostic(
  ...
  if (!response.ok) {
    ...
    throw new YoutubeAudioProviderError(`Provider returned HTTP ${response.status}`, {
