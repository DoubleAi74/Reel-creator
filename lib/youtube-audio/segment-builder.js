...
export async function buildYoutubeAudioSegment(job, providerResult, options = {}) {
  ...
  try {
    ...
    // Provider already cut the segment (e.g. youtube-mp36). Store as-is without
    // local ffmpeg/ffprobe — required on stock Vercel images.
    if (providerResult.trimMode === "provider") {
      const isMp3 = looksLikeMp3({
        mediaFormat: providerResult.mediaFormat,
        contentType: mediaResult.contentType,
      });

      if (!isMp3 && !canUseLocalFfmpeg()) {
        const error = new Error(
          `Provider returned non-MP3 media (${providerResult.mediaFormat || mediaResult.contentType || "unknown"}) and this host has no ffmpeg to convert it.`,
        );
        error.errorCode = "FFMPEG_MISSING";
        throw error;
      }
      ...
      // Non-MP3 with local ffmpeg available: fall through to convert path.
    }
