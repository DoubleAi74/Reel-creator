...
export function areAudioBinariesConfigured() {
  ...
  // Absolute/relative paths can be checked; bare names depend on PATH at spawn time.
  ...
  if (ffprobe.includes("/") || ffprobe.includes("\\")) {
...
export function ensureAudioBinariesAvailable() {
  if (!areAudioBinariesConfigured()) {
    throw new AudioProcessingError(
      "ffmpeg/ffprobe binaries are not available on this host. Set FFMPEG_PATH and FFPROBE_PATH, or use a provider-trimmed MP3 path.",
