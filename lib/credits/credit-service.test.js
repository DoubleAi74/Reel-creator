...
describe("credit service", () => {
  ...
  it("returns disabled shapes without touching Mongo when credits are off", async () => {
    ...
    await expect(assertCanStartGeneration({ phase: "generate" })).resolves.toEqual({
      enabled: false,
    });
    await expect(
      settlePhase({
  ...
  it("prechecks pricing and minimum balance before generation start", async () => {
    ...
    await expect(assertCanStartGeneration({ phase: "generate" })).resolves.toMatchObject({
      balanceMinor: 500,
      currency: "GBP",
      enabled: true,
    });
  ...
  it("gates only block-boundary phases and exempts enrich", async () => {
    ...
    expect(isBlockBoundaryPhase("generate")).toBe(true);
    ...
    await expect(assertCanStartGeneration({ phase: "generate" })).rejects.toMatchObject({
      code: "INSUFFICIENT_BALANCE",
    });
    await expect(assertCanStartGeneration({ phase: "time" })).rejects.toMatchObject({
      code: "INSUFFICIENT_BALANCE",
  ...
  it("keeps precheck models in sync with live pipeline models and priced (REP-206)", () => {
    ...
    for (const phase of ["transcribe", "enrich", "time", "generate", "full"]) {
