...
import { normalizeGenerationSave } from "./generations/save-eligibility.js";
...
// v2 envelopes without generationSave still decode (field optional).
...
function normalizeTranscription(transcription) {
  ...
  // Which client apply path the job feeds. Legacy v1 pointers used
  // mode="lyrics"|"timing"; keep the mapping here for dev-time restored
  // envelopes while v1 persisted envelopes are discarded by AUTOSAVE_VERSION.
  const rawPhase =
    typeof transcription.phase === "string"
      ? transcription.phase.trim()
      : typeof transcription.mode === "string"
        ? transcription.mode.trim()
        : "";
  const phase =
    rawPhase === "lyrics"
      ? "generate"
      : rawPhase === "timing"
        ? "time"
        : ["full", "generate", "transcribe", "enrich", "time"].includes(rawPhase)
          ? rawPhase
          : "full";
...
export function encodeAutosave({
  audioAsset,
  generationSave = null,
  project,
  transcription,
  ...
  const normalizedGenerationSave = normalizeGenerationSave(generationSave);
  const envelope = {
    audioAsset: normalizedAudioAsset,
    // Persist save eligibility across reloads (job ids + status).
    generationSave:
      normalizedGenerationSave.status === "idle" &&
      !normalizedGenerationSave.finalJobId
        ? null
        : {
            assetId: normalizedGenerationSave.assetId,
            finalJobId: normalizedGenerationSave.finalJobId,
            generationId: normalizedGenerationSave.generationId,
            pipelineRunId: normalizedGenerationSave.pipelineRunId,
            // Don't persist transient "saving" across reload.
            status:
              normalizedGenerationSave.status === "saving"
                ? "ready"
                : normalizedGenerationSave.status === "error"
                  ? "ready"
                  : normalizedGenerationSave.status,
          },
...
export function decodeAutosave(raw) {
  ...
  return {
    ...
    generationSave: normalizeGenerationSave(envelope.generationSave),
    project,
