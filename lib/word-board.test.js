...
describe("prepareBoardLines", () => {
  ...
  it("preserves line and word timing metadata", () => {
    const lines = prepareBoardLines([
      {
        id: "l1",
        end: 2.7,
        ...
        start: 1.2,
        words: [
          { text: "one", start: 1.24, end: 1.52 },
          { text: "two", start: 1.72, end: 2.05 },
        ],
      },
    ]);
  ...
  it("keeps repeated same text words distinct with source word keys", () => {
    const lines = prepareBoardLines([
      {
        id: "repeat-line",
        original: "go go",
        words: [
          { text: "go", start: 0.1, end: 0.3 },
          { text: "go", start: 0.4, end: 0.6 },
        ],
      },
    ]);

    expect(lines[0].words.map((word) => word.original)).toEqual(["go", "go"]);
    expect(lines[0].words.map((word) => word.sourceWordKey)).toEqual([
      "repeat-line:0",
      "repeat-line:1",
    ]);
  });
...
describe("measureTileWidth", () => {
  ...
  it("uses tighter mobile width limits and padding", () => {
    expect(measureTileWidth({ original: "i", english: "" }, { isMobile: true })).toBe(34);
    expect(
      measureTileWidth(
        { original: "abc", english: "def" },
        { isMobile: true, measureText: (text) => text.length * 10 },
      ),
    ).toBe(45);
    expect(
      measureTileWidth(
        { original: "veryveryverylongword", english: "alsolong" },
        { isMobile: true },
      ),
...
describe("estimateLineWidth", () => {
  ...
  it("uses the mockup's snug mobile tile widths", () => {
    const line = { words: [{ original: "a" }, { original: "b" }] };
    expect(estimateLineWidth(line, { boardScale: 1, isMobile: true })).toBe(79);
  });
});
...
describe("fitLayoutScale", () => {
  ...
  it("does not shrink below tileScale on mobile", () => {
    const lines = [
      { words: Array.from({ length: 30 }, () => ({ original: "word" })) },
    ];
    expect(
      fitLayoutScale(lines, {
        tileScale: 0.82,
        availableWidth: 200,
        isMobile: true,
      }),
...
describe("tile scale limits", () => {
  it("uses the mockup mobile defaults and keeps desktop unchanged", () => {
    expect(getTileScaleDefault(true)).toBe(0.82);
    expect(getTileScaleLimits(true)).toEqual({ min: 0.64, max: 1 });
    expect(getTileScaleDefault(false)).toBe(1);
    expect(getTileScaleLimits(false)).toEqual({ min: 0.82, max: 1.28 });
  });
});
...
describe("page/scroll line builders", () => {
  ...
  it("packs page-mode lines by each line's actual wrapped height", () => {
    ...
    expect(variable.lines).toHaveLength(2);
    expect(variable.pageCount).toBe(3);
    expect(variable.lines.map((line) => line.original)).toEqual(["a", "b"]);
  });

  it("returns a source-line page map for follow paging", () => {
    const result = buildPageLinesByHeight(lines, {
      availableHeight: 125,
      availableWidth: 400,
      boardScale: 1,
      tileScale: 1,
    ...
    expect(result.sourceLinePageMap.get("l0")).toBe(0);
    expect(result.sourceLinePageMap.get("l2")).toBe(2);
    expect(result.sourceLinePageMap.get("l6")).toBe(6);
  });
  ...
  it("buildScrollLines preserves timing metadata and source word keys", () => {
    const source = prepareBoardLines([
      {
        id: "timed-line",
        end: 3,
        original: "same same",
        start: 1,
        words: [
          { text: "same", start: 1.1, end: 1.4 },
          { text: "same", start: 1.5, end: 1.9 },
        ],
      },
    ]);
    const scroll = buildScrollLines(source);

    expect(scroll[0]).toMatchObject({
      id: "scroll-0",
      sourceId: "timed-line",
      start: 1,
      end: 3,
    });
    expect(scroll[0].words.map((word) => word.id)).toEqual([
      "scroll-0-0",
      "scroll-0-1",
    ]);
    expect(scroll[0].words.map((word) => word.sourceWordKey)).toEqual([
      "timed-line:0",
      "timed-line:1",
    ]);
    expect(scroll[0].words.map((word) => [word.start, word.end])).toEqual([
      [1.1, 1.4],
      [1.5, 1.9],
    ]);
  });
