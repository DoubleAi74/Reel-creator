...
export async function persistGeneration({
  ...
  // Session assets live under /tmp (ephemeral on Vercel). Missing files must not
  // fail reference-only saves (YouTube URL + lyrics in the snapshot).
  let assetMetadata = null;
  if (sessionId && assetId) {
    try {
      assetMetadata = await readAssetMetadata(sessionId, assetId);
    } catch {
      assetMetadata = null;
    }
  }
