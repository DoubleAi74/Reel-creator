...
export async function persistGeneration({
  ...
  const generationTitle = inferTitle({
    assetMetadata,
  ...
  const generation = await runTransaction(async (mongoSession) => {
  ...
  const assetFilePath = sessionId && assetId ? await getAssetFilePath(sessionId, assetId) : null;
  const r2Result = assetFilePath
    ? await putGenerationAudioObject({
        contentType: assetMetadata?.mimeType ?? "audio/mpeg",
