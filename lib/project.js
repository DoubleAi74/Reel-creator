...
import { repairMojibakeDeep } from "./text-encoding";
...
export function importProjectJson(jsonText) {
  // Heal any double-encoded (UTF-8-as-Latin-1) text before validation, so an
  // externally-corrupted file imports as readable Devanagari rather than garble.
  return importProjectValue(repairMojibakeDeep(parseProjectJson(jsonText)));
}
