...
const generationSchema = new mongoose.Schema(
  {
    ...
    // REP-403 / D-C: true only when the user supplied a non-empty title to publish.
    userTitled: {
      type: Boolean,
      default: false,
      required: true,
...
// REP-403 / D-C: public only when saved AND user-supplied title.
generationSchema.pre("validate", function syncPublicFlag() {
  if (
    this.isNew ||
    this.isModified("saved") ||
    this.isModified("userTitled") ||
    this.isModified("title")
  ) {
    this.public = this.saved === true && this.userTitled === true;
  }
});
...
// REP-501: unique partial index on finalJobId for concurrent persist safety.
generationSchema.index(
  { finalJobId: 1 },
  {
    partialFilterExpression: {
      finalJobId: { $type: "string" },
    },
    unique: true,
  },
);
